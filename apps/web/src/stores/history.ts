...
import {
  ...
  loadHistoryFromDb,
  updateHistoryInDb,
} from '../store/historyDb'
import type { HistorySnapshot, HistorySnapshotInput } from '../store/historyTypes'

export type { HistorySnapshot } from '../store/historyTypes'
...
export const useHistoryStore = defineStore('history', () => {
  ...
  const pruneMissingFiles = async (entries: HistorySnapshot[]) => {
    ...
    const checks = await Promise.all(
      entries.map((entry) =>
        entry.filePath
          ? electronFile.fileExists(entry.filePath).catch(() => ({ exists: true }))
  ...
  const updateEntryState = async (id: string, updates: Partial<HistorySnapshot>) => {
    ...
    if (!hasChanges(history.value[index], updates)) {
    ...
    history.value = refreshOrder(
      history.value.map((entry, idx) => (idx === index ? updated : entry))
    )
    return updated
  }
  ...
  async function saveSnapshot(
    ...
    if (shouldCreateHistory) {
      ...
      history.value = refreshOrder([historyEntry, ...history.value]).slice(0, MAX_HISTORY_ENTRIES)
  ...
  async function deleteEntry(id: string) {
    ...
    history.value = history.value.filter((e) => e.id !== id).slice(0, MAX_HISTORY_ENTRIES)
    if (activeId.value === id) {
      activeId.value = history.value[0]?.id ?? null
    }
  }

  async function clearHistory() {
    const filePaths = history.value.map((entry) => entry.filePath).filter((p): p is string => !!p)
