...
function adjustBackgroundBrightness(
  ...
  if (
    (s === 0 && l > CONSTANTS.HIGH_BLACKWHITE_HSL_BRIGHTNESS) ||
...
function adjustCodeTextBrightness(
  ...
  if (l > 70)
    return adjustBrightnessTo(
      Math.max(200, getColorPerceivedBrightness(textRgb)),
      textRgb
    );
...
function transformDeclarations(selector: string, props: string): string {
  ...
  for (const d of parsedDecls) {
    if (
      (d.lowerName === "background-color" || d.lowerName === "background") &&
      !d.val.toLowerCase().includes("url(") &&
      !d.val.toLowerCase().includes("gradient")
    ) {
